...
export function formatOutputDelta(output: Pick<ScenarioOutput, 'current' | 'baseline' | 'invert'>): DeltaDisplay {
  ...
  const sign = diff > 0 ? '+' : '';
  const text = atBaseline ? 'At baseline' : `${sign}${pct.toFixed(1)}% vs baseline`;
